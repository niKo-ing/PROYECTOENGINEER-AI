...
export function ProductCard({ product }: { product: ProductRead }) {
  const hasPrice = product.lowest_price !== null;
  const hasImages = product.images.length > 0;

  return (
    <Link href={`/product/${product.id}`} className="group block h-full focus-visible:outline-none">
      <Card className="flex h-full flex-col overflow-hidden border-muted bg-card transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg">
