...
class AIOrchestrator:
    ...
    def chat(
        ...
        prompt = self._build_prompt(message, product_id, intent)
        try:
        ...
        report = self._run_research(message, intent, products, comparison)
        ...
        if not initial.tool_calls:
            ...
            return self._response(initial, [], initial, products, comparison, intent, sources=sources, research=report is not None)

        evidence_prompt = self._augment_prompt(prompt, products, comparison, research_context=research_context)
        ...
        return self._response(final, used_tools, initial, products, comparison, intent, sources=sources, research=report is not None)
    ...
    def _run_research(self, message: str, intent: AIIntent, products: list[dict], comparison: dict[str, Any] | None) -> ResearchReport | None:
        ...
        if self.research_service is None:
            self.research_service = WebResearchService.from_settings()
        targets = _to_research_targets(products)
        report = self.research_service.research(targets, message)
        return report if report.used else None
    ...
    def _should_research(self, message: str, intent: AIIntent, products: list[dict], comparison: dict[str, Any] | None) -> bool:
        ...
        if not _has_target_identity(targets):
            return False
        if _wants_external_information(message):
            return True
        return _has_spec_gaps(targets, comparison)
    ...
    @staticmethod
    def _augment_prompt(prompt: str, products: list[dict], comparison: dict[str, Any] | None, research_context: str | None = None) -> str:
        lines = []
        if products:
            summary = " | ".join(
        ...
        block = "\n".join(lines)
        if research_context:
            block = f"{block}\n{research_context}" if block else research_context
        if not block:
            return prompt
        return prompt + "\n\n[Evidencia determinística — usala al redactar la respuesta, sin inventar datos]\n" + block

    # ── Response helpers ────────────────────────────────────────────────

    def _response(self, final, used_tools: list[str], initial, products: list[dict], comparison: dict[str, Any] | None, intent: AIIntent, sources: list[dict] | None = None, research: bool = False) -> ChatResponse:
